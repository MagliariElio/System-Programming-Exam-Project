...
impl base::ResolveWireError for Error {
    unsafe fn resolve_wire_error(first_error: u8, raw: *mut xcb_generic_error_t) -> Self {
        ...
        match error_code - first_error {
            0 => Error::PictFormat(PictFormatError::from_raw(raw)),
            1 => Error::Picture(PictureError::from_raw(raw)),
            2 => Error::PictOp(PictOpError::from_raw(raw)),
            3 => Error::GlyphSet(GlyphSetError::from_raw(raw)),
            4 => Error::Glyph(GlyphError::from_raw(raw)),
            _ => unreachable!(
                "Could not resolve render Error with error_code {} and first_error {}",
