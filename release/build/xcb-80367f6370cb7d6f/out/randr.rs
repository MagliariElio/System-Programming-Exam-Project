...
impl base::ResolveWireError for Error {
    unsafe fn resolve_wire_error(first_error: u8, raw: *mut xcb_generic_error_t) -> Self {
        ...
        match error_code - first_error {
            0 => Error::BadOutput(BadOutputError::from_raw(raw)),
            1 => Error::BadCrtc(BadCrtcError::from_raw(raw)),
            2 => Error::BadMode(BadModeError::from_raw(raw)),
            3 => Error::BadProvider(BadProviderError::from_raw(raw)),
            _ => unreachable!(
                "Could not resolve randr Error with error_code {} and first_error {}",
...
impl base::ResolveWireEvent for Event {
    unsafe fn resolve_wire_event(first_event: u8, raw: *mut xcb_generic_event_t) -> std::option::Option<Self> {
        ...
        match response_type - first_event {
            0 => Some(Event::ScreenChangeNotify(ScreenChangeNotifyEvent::from_raw(raw))),
            1 => Some(Event::Notify(NotifyEvent::from_raw(raw))),
            _ => None,
        }
    }
}
