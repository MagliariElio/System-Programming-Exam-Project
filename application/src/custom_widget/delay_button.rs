...
pub const DELAY_SCREENSHOT: Selector<(Rect, WindowId, WidgetId, WidgetId, Box<str>, Box<str>, ImageFormat, u64,u8)> = Selector::new("Save the screenshot image, last param: where to save");
        ...
        match event {
            Event::Command(cmd) => {
                if cmd.is(DELAY_SCREENSHOT) {
                    ...
                    self.taking_screenshot = Some((*rect,*main_window_id,*custom_zstack_id,*screenshot_id,path.clone(),file_name.clone(),*file_format,d, *monitor));
                    ctx.request_layout();
                }
            }
            Event::MouseDown(_) => {
