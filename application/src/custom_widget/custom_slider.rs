...
use druid::{theme, Color, KeyOrValue, LinearGradient, Point, Rect, UnitPoint, Vec2, WidgetPod, Target};
use druid::kurbo::{PathEl, Shape};
use druid::piet::{PietText, PietTextLayout, Text, TextLayout, TextLayoutBuilder};
use tracing::{instrument, trace, warn};
use crate::custom_widget::UPDATE_COLOR;
...
impl CustomSlider {
    ...
    pub fn set_zstack_id(&mut self, zstack_id: WidgetId){
        self.zstack_id = Some(zstack_id);
    }
}

impl Widget<f64> for CustomSlider {
    ...
    fn event(&mut self, ctx: &mut EventCtx, event: &Event, data: &mut f64, env: &Env) {
        if !ctx.is_disabled() {
            ...
            } else if let Event::MouseUp(_me) = event {
                ctx.get_external_handle().submit_command(UPDATE_COLOR,(None,Some(*data)),Target::Widget(self.zstack_id.unwrap())).unwrap();
            }
        }
    }
